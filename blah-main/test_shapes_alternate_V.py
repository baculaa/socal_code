...
class Initiator:
    ...
    ##############################################################
    # THE FOLLOWING METHODS DEFINE THE DIFFERENT SHAPES THE ROBOTS CAN MOVE IN
    #
    # Specifically, it gives the x and y coordinates fro each robot to move
    # and defines the x,y direction the robot should face when it has reached
    # that position
    #
    # THINGS TO NOTE:
    #   - Positive x is forward
    #   - Positive y is to the left
    #   - orientation variable set to face forward
    #
    #############################################################
    ...
    def line(self, shape, x_ref, y_ref, side_length, num_rob, orientation):
        ...
        lineGoal_base2 = [x1,y1,x2,y2,x3,y3]
        ...
        if shape == 4:
            ...
            goal_rot1 = self.rotate_around_point(lineGoal_base2[0],lineGoal_base2[1],0,0,orientation)
            goal_rot2 = self.rotate_around_point(lineGoal_base2[2],lineGoal_base2[3],0,0,orientation)
            goal_rot3 = self.rotate_around_point(lineGoal_base2[4],lineGoal_base2[5],0,0,orientation)
